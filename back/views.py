...
def notice():
    if request.method == 'GET':
        notice = ArticleType.query.all()
    return render_template('back/notice.html', notice=notice)


@back_blueprint.route('/comment/', methods=['GET','POST'])
def comment():
    if request.method == 'GET':
        comment = ArticleType.query.all()
    return render_template('back/comment.html',comment=comment)


@back_blueprint.route('/flink/', methods=['GET','POST'])
def flink():
    if request.method == 'GET':
        flink = ArticleType.query.all()
    return render_template('back/flink.html', flink=flink)


@back_blueprint.route('/loginlog/', methods=['GET','POST'])
def loginlog():
    if request.method == 'GET':
        loginlog = ArticleType.query.all()
    return render_template('back/loginlog.html', loginlog=loginlog)


@back_blueprint.route('/manage_user/', methods=['GET','POST'])
def manage_user():
    if request.method == 'GET':
        manage_user = ArticleType.query.all()
    return render_template('back/manage-user.html', manage_user=manage_user)


@back_blueprint.route('/setting/', methods=['GET','POST'])
def setting():
    if request.method == 'GET':
        setting = ArticleType.query.all()
    return render_template('back/setting.html', setting=setting)


@back_blueprint.route('/readset/', methods=['GET','POST'])
def readset():
    if request.method == 'GET':
        readset = ArticleType.query.all()
    return render_template('back/readset.html', readsets=readset)
